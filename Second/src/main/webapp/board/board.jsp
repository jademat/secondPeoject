...
<div class="container mt-3">
	...
	<table class="table table">
		<tr align="center">
			...
			<th><a href="<%=request.getContextPath()%>/user_board_list.go?board_type=notice">공지사항</a></th>
			<th><a href="<%=request.getContextPath()%>/user_board_list.go?board_type=exchange">교환</a></th>
			<th><a href="<%=request.getContextPath()%>/user_board_list.go?board_type=refund">환불</a></th>
			<th><a href="<%=request.getContextPath()%>/user_board_list.go?board_type=inquiry">상품문의</a></th>
		</tr>
	</table>
					...
					<td>
						<c:choose>
							<c:when test="${dto.board_visible == 0}">
								<c:choose>
									<c:when test="${dto.getUser_id() == sessionScope.user_id || sessionScope.user_id == 'admin'}">
										${dto.board_title}
									</c:when>
									<c:otherwise>
										비공개글입니다.
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${dto.board_type == 'notice'}">
										<span style="color: red; font-weight: bold;">${dto.board_title}</span>
									</c:when>
									<c:otherwise>
										${dto.board_title}
									</c:otherwise>
...
<div class="container">
	<ul class="pagination">
		...
		<c:if test="${endBlock < allPage}">
			<li class="page-item"><a class="page-link"
				href="user_board_list.go?board_type=${board_type }&page=${endBlock + 1 }">Next</a></li>
		</c:if>
	</ul>
</div>
